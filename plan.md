...
- 已实现 `AnalysisProvider` 接口和开发 Provider，先保证产品闭环与结构化输出；真实大模型 Provider 尚未接入，所以该阶段首项暂不勾选完成。
- 已实现 Tab A 结果概览的 3 张方向卡和 7 个必填字段；详细报告与 7 天计划尚未完成，所以 Tab A 完整项暂不勾选。
- 输入过短或缺资金/时间等关键信息时返回最多 3 个追问；完整两轮追问交互尚未完成。
- 分析会话写入 `analysis_sessions`，支持后续历史记录和报告恢复的服务端基础；历史列表和详细报告页尚未完成。
- 前端 `/analysis` 已替换占位页，支持提交需求、展示追问和 3 张方向卡；视觉仍需按 `docs/design-references/cdk-redemption/` 重做。
