...
export default function SandboxRunView({ run, onStop, onRetryRole }: Props) {
  ...
  const completed = rows.filter((role) => role.status === "done").length; const active = run.status === "running"; const terminal = ["done", "partial", "failed", "ai_no_result"].includes(run.status);
