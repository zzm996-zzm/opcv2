...
describe("ToolsPage", () => {
  ...
  it("renders an empty tool library instead of static sample tools", async () => {
    ...
    expect(await screen.findByText("暂无工具数据")).toBeInTheDocument();
  ...
  it("renders the full tool library empty state", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ tools: [] }), { status: 200 })
    );
    renderPage("all");
  ...
  it("activates the left category rail without showing fallback tools", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ tools: [] }), { status: 200 })
    );
    renderPage();
    ...
    await waitFor(() => expect(screen.queryByText("SimilarWeb")).not.toBeInTheDocument());
  });
