...
func (r *PostgresRepository) FailV2RunStart(ctx context.Context, userID, runID int64, errorCode string) error {
	_, err := r.db.Exec(ctx, `
		UPDATE sandbox_sessions SET status = $1, current_step = $2, error_message = $3,
		v2_status = $2, v2_finished_at = NOW(), updated_at = NOW()
		WHERE user_id = $4 AND id = $5 AND sandbox_version = 2 AND v2_status = $6
	`, StatusFailed, V2StatusFailed, errorCode, userID, runID, V2StatusRunning)
	return err
}
