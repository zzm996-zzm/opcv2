...
type LearningContextProvider interface {
	GetCourse(ctx context.Context, slug string) (learning.Course, error)
	GetProgress(ctx context.Context, userID int64, courseSlug string) (learning.Progress, error)
	ListProgress(ctx context.Context, userID int64) ([]learning.Progress, error)
	LatestDiagnosis(ctx context.Context, userID int64) (learning.Diagnosis, error)
...
type CRMContextProvider interface {
	GetCustomer(ctx context.Context, userID, customerID int64) (crm.Customer, error)
	ListActivities(ctx context.Context, userID, customerID int64, limit int) ([]crm.Activity, error)
	ListFollowUps(ctx context.Context, input crm.ListFollowUpsInput) ([]crm.FollowUp, error)
...
type learningContext struct {
	...
	Course     *learning.Course        `json:"course,omitempty"`
	Progress   *learning.Progress      `json:"progress,omitempty"`
...
type crmContext struct {
	...
	Activities []crm.Activity     `json:"activities,omitempty"`
	FollowUps  []crm.FollowUp     `json:"follow_ups,omitempty"`
	DueToday   []crm.Customer     `json:"due_today,omitempty"`
	Pipeline   *crm.PipelineStats `json:"pipeline,omitempty"`
}

type profileContext struct {
	Profile account.ProfileContext `json:"profile"`
}
...
func (s *Service) StreamMessage(ctx context.Context, input SendMessageInput, onEvent func(StreamEvent) error) (SendMessageResult, error) {
	...
	learningContext, err := s.loadLearningContext(ctx, input)
	if err != nil {
		return SendMessageResult{}, err
	}
	...
	crmContext, err := s.loadCRMContext(ctx, input)
	if err != nil {
		return SendMessageResult{}, err
	}
	profileContext, err := s.loadProfileContext(ctx, input)
	if err != nil {
		return SendMessageResult{}, err
	}
	...
	streamResult, err := s.streamer.GenerateTextStream(ctx, ai.GenerateTextRequest{
		...
		UserPrompt:   buildUserPrompt(thread, memories, messages, references, taskContext, competitorContext, growthContext, monitoringContext, projectContext, learningContext, sandboxContext, crmContext, profileContext, content, false),
...
func (s *Service) loadLearningContext(ctx context.Context, input SendMessageInput) (*learningContext, error) {
	filters := normalizeContextFilters(input.ActiveFilters)
	if filters["module"] != "learning" {
		return nil, nil
	}
	if s.learning == nil {
		return nil, ErrServiceNotReady
	}
	context := &learningContext{View: strings.TrimSpace(filters["view"])}
	courseSlug := strings.TrimSpace(filters["course_slug"])
	if courseSlug != "" {
		course, err := s.learning.GetCourse(ctx, courseSlug)
		if err != nil && !errors.Is(err, learning.ErrCourseNotFound) {
			return nil, err
		}
		if err == nil {
			context.Course = &course
		}
		progress, err := s.learning.GetProgress(ctx, input.UserID, courseSlug)
		if err != nil && !errors.Is(err, learning.ErrProgressNotFound) {
			return nil, err
		}
		if err == nil {
	...
	if context.View == "history" {
		progresses, err := s.learning.ListProgress(ctx, input.UserID)
		if err != nil {
			return nil, err
		}
		if len(progresses) > 20 {
			progresses = progresses[:20]
		}
		context.Progresses = progresses
	}
	diagnosis, err := s.learning.LatestDiagnosis(ctx, input.UserID)
	if err != nil && !errors.Is(err, learning.ErrDiagnosisNotFound) {
		return nil, err
	}
	if err == nil {
		context.Diagnosis = &diagnosis
		if context.View == "plan" {
			plan, err := s.learning.GetPlan(ctx, input.UserID, diagnosis.ID)
			if err != nil {
				return nil, err
			}
			context.Plan = &plan
...
func (s *Service) loadCRMContext(ctx context.Context, input SendMessageInput) (*crmContext, error) {
	filters := normalizeContextFilters(input.ActiveFilters)
	...
	context := &crmContext{View: strings.TrimSpace(filters["view"])}
	customerIDValue := strings.TrimSpace(filters["customer_id"])
	if customerIDValue != "" {
		customerID, err := strconv.ParseInt(customerIDValue, 10, 64)
		if err != nil || customerID <= 0 {
			return nil, ErrInvalidInput
		}
		customer, err := s.crm.GetCustomer(ctx, input.UserID, customerID)
		if err != nil {
			return nil, err
		}
		context.Customer = &customer
		activities, err := s.crm.ListActivities(ctx, input.UserID, customerID, 20)
		if err != nil {
			return nil, err
		}
		context.Activities = activities
		followUps, err := s.crm.ListFollowUps(ctx, crm.ListFollowUpsInput{UserID: input.UserID, CustomerID: customerID, Limit: 20})
		if err != nil {
			return nil, err
		}
		context.FollowUps = followUps
	}
	if context.View == "overview" || context.View == "customers" || context.View == "" {
		pipeline, err := s.crm.PipelineStats(ctx, input.UserID)
		if err != nil {
			return nil, err
		}
		context.Pipeline = &pipeline
		if customerIDValue == "" {
			dueToday, err := s.crm.ListDueCustomers(ctx, crm.ListDueInput{UserID: input.UserID, Limit: 20})
			if err != nil {
				return nil, err
			}
			context.DueToday = dueToday
		}
	}
	return context, nil
}

func (s *Service) loadProfileContext(ctx context.Context, input SendMessageInput) (*profileContext, error) {
	if s.profile == nil {
		return nil, nil
	}
	profile, err := s.profile.GetProfileContext(ctx, input.UserID)
	...
	return &profileContext{Profile: profile}, nil
}
...
func WithCRMContextProvider(provider CRMContextProvider) Option {
	return func(service *Service) {
...
func (s *Service) CompareMessages(ctx context.Context, input CompareMessagesInput) (CompareMessagesResult, error) {
	...
	profileContext, err := s.loadProfileContext(ctx, SendMessageInput{UserID: input.UserID})
	if err != nil {
		return CompareMessagesResult{}, err
	}
...
func (s *Service) generateReply(ctx context.Context, userID int64, thread Thread, content, model string, references []File, taskContext *TaskContext, competitorContext *competitorScanContext, growthContext *growthModelContext, monitoringContext *monitoringContext, projectContext *projectContext, learningContext *learningContext, sandboxContext *sandboxContext, crmContext *crmContext, profileContext *profileContext) (ai.GenerateJSONResult, chatAIResult, error) {
	...
	aiResult, err := s.generator.GenerateJSON(ctx, ai.GenerateJSONRequest{
		...
		UserPrompt:     buildUserPrompt(thread, memories, messages, references, taskContext, competitorContext, growthContext, monitoringContext, projectContext, learningContext, sandboxContext, crmContext, profileContext, content, true),
...
func buildUserPrompt(thread Thread, memories []Memory, messages []Message, references []File, taskContext *TaskContext, competitorContext *competitorScanContext, growthContext *growthModelContext, monitoringContext *monitoringContext, projectContext *projectContext, learningContext *learningContext, sandboxContext *sandboxContext, crmContext *crmContext, profileContext *profileContext, content string, includeResponseSchema bool) string {
	...
	if profileContext != nil {
		payload, err := json.Marshal(profileContext)
		if err == nil {
			builder.WriteString("\n用户维护的业务档案（用户主动填写，已按当前用户权限读取；不代表 AI 生成的长期记忆）：")
	...
	if learningContext != nil {
		payload, err := json.Marshal(learningContext)
		if err == nil {
	...
	if crmContext != nil {
		payload, err := json.Marshal(crmContext)
		if err == nil {
			builder.WriteString("\n当前 CRM 上下文（客户、活动、跟进和概览统计已按当前用户权限读取）：")
			builder.WriteString(truncateForPrompt(string(payload), 16000))
			builder.WriteString("\n")
		}
	}
